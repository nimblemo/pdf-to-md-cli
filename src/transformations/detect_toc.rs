use crate::models::{BlockType, ParseResult, ItemType, LineItem, TextItem};
...
impl Transformation for DetectTOC {
    fn transform(&self, result: &mut ParseResult) {
        ...
        for (page_idx, page) in result.pages.iter().enumerate().take(max_pages_to_evaluate) {
            ...
            for (line_idx, item) in page.items.iter().enumerate() {
                if let ItemType::LineItem(line) = item {
                    ...
                    if page_idx == 6 && self.verbose {
                         eprintln!("DEBUG: Page 6 Line raw: {:?}", line.items.iter().map(|w| &w.text).collect::<Vec<_>>());
                    }
                    ...
                    if let Some(last_idx) = words.len().checked_sub(1) {
                        ...
                        if !suffix_digits.is_empty() {
                            ...
                            if let Some(pos) = original_text.rfind(&digits) {
                                ...
                                let clean_prefix = prefix.trim_end_matches(|c| c == '.' || c == ' ');
                    ...
                    if page_idx == 6 && self.verbose {
                         ...
                         if !digits.is_empty() || text.contains("First Things First") {
                             ...
                             if let Some(last_idx) = words.len().checked_sub(1) {
                                 ...
                                 let trimmed = original.trim_end_matches(|c| c == '*' || c == '_' || c == ' ');
                    ...
                    if ends_with_digit {
                        if let Some(prev_words) = last_words_without_number.take() {
                             ...
                             if page_idx == 6 && self.verbose {
                                 eprintln!("DEBUG: Merge check. gap={:.2}, threshold={:.2}", gap, threshold);
                             }
                        ...
                        if self.verbose {
                             let text = link_line_item.items.iter().map(|w| w.text.as_str()).collect::<String>();
                             if text.contains("Learning from Failure") || text.contains("Talking About Failure") || text.contains("Postincident Reviews") {
                                 eprintln!("DEBUG: TOC Item '{}' X={:.2}", text, link_line_item.x);
                             }
                             if link_line_item.x > 77.0 && link_line_item.x < 80.0 {
                                 eprintln!("DEBUG: Found item at X={:.2}: '{}'", link_line_item.x, text);
                             }
                        }
            ...
            if processed_items > 0 && (line_items_with_digits as f64 * 100.0 / processed_items as f64) > 75.0 {
                ...
                if self.verbose {
                    eprintln!("DEBUG: Detected TOC page {}", page.index);
                }
            }
        }
            ...
            if let Some(page) = result.pages.get_mut(page_idx) {
                ...
                for &idx in unknown_lines {
                    let line_text = match &page.items[idx] {
                        ItemType::LineItem(item) => item.items.iter().map(|i| i.text.as_str()).collect::<String>(),
                        _ => String::new(),
                    };
                    ...
                    let check_repetitive = |text: &str| -> bool {
                         if first_page_headers.contains(text) { return true; }
                         ...
                         if first_page_headers.iter().any(|h| h.len() > 3 && text.starts_with(h)) { return true; }
                         
                         // Check normalized version (split by |)
                         let text_norm = text.split('|').next().unwrap_or(text).trim();
                         if first_page_headers.contains(text_norm) { return true; }
                         if first_page_headers.iter().any(|h| h.len() > 3 && text_norm.starts_with(h)) { return true; }
                    ...
                    if seq_idx > 0 && check_repetitive(clean_line_text) {
                        if self.verbose {
                            eprintln!("DEBUG: Removing repetitive TOC header (dynamic): '{}'", clean_line_text);
                        }
                        continue;
                    }
                    ...
                    if seq_idx > 0 && clean_line_text.to_lowercase().contains("table of contents") {
                        if self.verbose {
                            eprintln!("DEBUG: Removing repetitive TOC header: '{}'", clean_line_text);
                        }
                        continue;
                    }

                    let is_duplicate = toc_links.iter().any(|link| {
                        let link_text = link.line_item.items.iter().map(|i| i.text.as_str()).collect::<String>();
                    ...
                    if !is_duplicate {
                        ...
                        if seq_idx == 0 {
                            ...
                            let norm = clean_line_text.split('|').next().unwrap_or(clean_line_text).trim();
                            if norm != clean_line_text {
                                first_page_headers.insert(norm.to_string());
                            }
                        }
                    } else if self.verbose {
                        eprintln!("DEBUG: Removing duplicate unknown line {}: '{}'", idx, clean_line_text);
                    }
                }
                ...
                for (i, item) in current_items.iter().enumerate() {
                     if let ItemType::LineItem(line) = item {
                        ...
                        let text = line.items.iter().map(|w| w.text.as_str()).collect::<String>();
                        ...
                        // Inline check:
                        let is_repetitive = {
                             if first_page_headers.contains(clean_text) { true }
                             else if first_page_headers.iter().any(|h| h.len() > 3 && clean_text.starts_with(h)) { true }
                             else {
                                 let text_norm = clean_text.split('|').next().unwrap_or(clean_text).trim();
                                 if first_page_headers.contains(text_norm) { true }
                                 else if first_page_headers.iter().any(|h| h.len() > 3 && text_norm.starts_with(h)) { true }
                                 else { false }
                             }
                        };
